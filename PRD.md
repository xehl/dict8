...
- On-device speech-to-text using WhisperKit (`distil-whisper/distil-large-v3`) on Apple Silicon ANE with automatic remote fallback, plus fast cloud cleanup model experimentation (approved exception)
...
**Approved exception — cleanup stage deadline and output cap lowered (approved 2026-08-13, updated 2026-08-19, updated 2026-08-25):** The cleanup request deadline is 5.0 seconds (adjusted from 3.5s to prevent false-positive dropouts during transient network variance, while default Nitro execution finishes in 250–500ms) and its `max_completion_tokens` cap is 1,024 (lowered from 2,048, with the per-input floor preserved at 128). Rationale: cleanup output should never be much longer than "Me but punctuated" input, so the worst-case generation cap and failure-detection latency both have slack to tighten; failing into the raw-transcript-fallback path after 5 seconds eliminates noticeable UI hangs. This does not change STT's deadline or token limits, and it does not authorize combining a same-model retry with fallback beyond the existing per-stage attempt cap.
...
**Approved exception — ephemeral local dictation and cleanup diagnostics (approved 2026-08-18, updated 2026-08-19):** To support live model benchmarking, validator rule calibration, and prompt inspection, the app retains an in-memory ring buffer (up to 3 entries, lifetime up to 10 minutes, mirroring `LastDictationCache` lifecycle: auto-cleared on screen lock, sleep, Quit, or manual clear) containing the raw input, cleaned candidate output, candidate model slug, and computed validation ratios for recent dictations and cleanup evaluations. These diagnostic entries are strictly local to process memory, never written to disk or logs, never synced, and exist solely to support model benchmarking and prompt quality tuning.

**Approved exception — custom vocabulary, target context, and spot-correction learning (approved 2026-08-18, updated 2026-08-19, extended 2026-08-21, hardened 2026-08-24):** The cleanup stage incorporates user-configured custom vocabulary (stored in local `UserDefaults`), originating application context (`PasteTarget` application name), focused window/document title, and preceding cursor text (up to 100 characters before insertion point) into the cleanup prompt and validator. Custom vocabulary terms are recognized as expected vocabulary during output validation so proper nouns and technical names are not flagged as novel content. Targets identified as code editors or terminals instruct the model to preserve camelCase, snake_case, variable names, and code syntax. Preceding cursor text provides contextual insertion awareness (preventing awkward capitalization or trailing punctuation when dictating mid-sentence). Additionally, after successful paste, dict8 observes the focused text field across a 10-second observation window with sliding-window buffer alignment and 2-to-1 phrase support (e.g. 'in physical' -> 'Infisical'); if a single-word or phrase fix is detected, the corrected term is automatically appended to custom vocabulary with non-intrusive HUD feedback.

**Approved exception — audio ducking and recorder pre-warming (approved 2026-08-21):** To ensure optimal speech recognition signal-to-noise ratio and eliminate recording startup latency, the audio subsystem initiates background audio ducking (attenuating external media playback during chord hold) and pre-warms the audio recorder pipeline upon hotkey engagement. Pre-warmed recording drivers and ambient ducking state are strictly ephemeral and teardown cleanly upon key release or cancellation. Local WhisperKit STT engine additionally supports streaming audio array transcription interfaces for direct in-memory sample evaluation.

**Approved exception — rolling-window latency routing and prompt KV caching (approved 2026-08-24):** To minimize cloud cleanup latency and enforce fast tail completion, cleanup completion requests specify provider routing preferences (`preferred_max_latency: {"p90": 1.5}`, `preferred_min_throughput: {"p90": 60}`) and send a consistent `X-Session-ID` header across requests to pin OpenRouter's 10-minute prompt KV cache. System prompts are structured with invariant instructions at the start and dynamic variables (target window, cursor context, raw transcript) at the tail to maximize KV prefix reuse.
