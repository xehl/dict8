...
**Approved exception — cleanup stage deadline and output cap lowered (approved 2026-08-13, updated 2026-08-19):** The cleanup request deadline is 3.5 seconds (lowered from 10 seconds, originally 30 seconds) and its `max_completion_tokens` cap is 1,024 (lowered from 2,048, with the per-input floor lowered from 64 to 48). Rationale: cleanup output should never be much longer than "Me but punctuated" input, so the worst-case generation cap and failure-detection latency both have slack to tighten; failing fast into the existing raw-transcript fallback path within 3.5s eliminates noticeable spinner lag on cold or slow remote model picks.
