...
* **Approved exception — explicit reasoning effort none for cleanup completions (approved 2026-08-13):** Cleanup requests include `"reasoning": {"effort": "none"}` in their request payload. Rationale: Auto Router candidates that support chain-of-thought (e.g. DeepSeek v4 Flash) generate reasoning tokens that count against `max_completion_tokens`. Clamping the output token limit for light editing previously caused reasoning models to reach `finish_reason: length` before emitting completion text, leading to false-positive `missingChoice` fallbacks. Explicitly setting `effort: none` ensures reasoning models immediately emit cleaned text without consuming the token budget on chain-of-thought. STT does not use chat completions and is unaffected.
* **Approved exception — hybrid on-device STT (WhisperKit) and fast cloud cleanup experimentation (approved 2026-08-15):** The app supports local on-device speech-to-text via `WhisperKit` (`distil-whisper/distil-large-v3` default) on Apple Silicon ANE alongside the remote OpenRouter STT provider, and supports candidate fast cloud cleanup models (e.g., `google/gemini-2.5-flash-lite`, `meta-llama/llama-3.1-8b-instruct:nitro`, `openai/gpt-4o-mini`) via an experimentation harness. Rationale: on-device STT provides $0 egress, strict on-device audio privacy, and ~100–150ms transcription latency; paired with low-latency cleanup candidates, this drives end-to-end latency down to sub-500ms. Local STT conforms to `SpeechToTextProviding` and transparently falls back to `OpenRouterSpeechToTextService` if hardware initialization fails.
* **Approved exception — ephemeral local cleanup failure diagnostics (approved 2026-08-18):** To diagnose text cleanup validation failures (such as false-positive excessive novel content and substantial expansion rejections across candidate cleanup models), the app may retain an in-memory ring buffer (up to 20 entries, lifetime up to 10 minutes, mirroring `LastDictationCache` lifecycle: auto-cleared on screen lock, sleep, Quit, or manual clear) containing the raw input, candidate output, candidate model slug, and computed validation ratios. These diagnostic entries are strictly local to process memory, never written to disk or logs, never synced, and exist solely to support model benchmarking and validator rule calibration.
* **Approved exception — custom vocabulary and target application context awareness (approved 2026-08-18):** The cleanup stage incorporates user-configured custom vocabulary (stored in local `UserDefaults`) and originating application context (`PasteTarget` application name) into the cleanup prompt and validator. Custom vocabulary terms are recognized as expected vocabulary during output validation so proper nouns and technical names are not flagged as novel content. Targets identified as code editors or terminals instruct the model to preserve camelCase, snake_case, variable names, and code syntax.
