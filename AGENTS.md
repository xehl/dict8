...
* Each AI stage has one pinned model and at most one explicit fallback, **except** the cleanup stage, which uses OpenRouter's `openrouter/auto` Auto Router per the explicit exception below.
...
* **Approved exception — cleanup stage uses OpenRouter Auto Router (approved 2026-08-12):** The cleanup stage sends `model: "openrouter/auto"` instead of a pinned primary model plus one explicit fallback. STT is unaffected and keeps its pinned primary/fallback pair. Rationale: cleanup is a low-stakes, low-cost, high-volume stage where OpenRouter's live spend-share routing can pick a suitable model without dict8 hand-maintaining a fallback list. Consequences of this exception:
