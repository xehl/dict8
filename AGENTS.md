...
* Each AI stage has one pinned model and at most one explicit fallback, **except** the cleanup stage, which uses OpenRouter's `openrouter/auto-beta` Auto Router (Beta track) per the explicit exception below.
...
* **Approved exception — cleanup stage uses OpenRouter Auto Router Beta track (approved 2026-08-12, switched to Beta 2026-08-13):** The cleanup stage sends `model: "openrouter/auto-beta"` instead of a pinned primary model plus one explicit fallback, and its per-request Auto Router settings use the `auto-beta-router` plugin id (the `auto-router` plugin id used by the stable slug is silently ignored on the beta track). STT is unaffected and keeps its pinned primary/fallback pair. Rationale: cleanup is a low-stakes, low-cost, high-volume stage where OpenRouter's live spend-share routing can pick a suitable model without dict8 hand-maintaining a fallback list; the Beta track gets new routing behaviors before they reach the stable slug. Consequences of this exception:
  * Cleanup makes exactly one explicit model attempt (`openrouter/auto-beta`); there is no local dict8-side model-fallback switch for cleanup. A transient failure on that single attempt is treated as an ordinary cleanup failure (raw transcript pastes, per existing failure behavior) rather than triggering a second model attempt.
